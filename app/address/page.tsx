...
const addresses = [
  {
    icon: <Home className="h-6 w-6 text-[#2b9bf4]" />,
    label: "HOME",
    value: "2464 Royal Ln. Mesa, New Jersey 45463",
  },
  {
    icon: <Briefcase className="h-6 w-6 text-[#9d5bef]" />,
    label: "WORK",
  ...
  return (
    <div className="flex min-h-[100dvh] flex-col bg-white pb-8">
      ...
      <div className="mt-6 flex-1 space-y-5 px-6">
        {addresses.map((a, i) => (
          <div key={i} className="flex gap-4 rounded-[20px] bg-surface px-4 py-5">
            <span className="flex h-[55px] w-[55px] shrink-0 items-center justify-center rounded-full bg-white">
              {a.icon}
            </span>
